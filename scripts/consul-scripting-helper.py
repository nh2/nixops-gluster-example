...
def waitForLeader(args):
  ...
  while True:
    try:
      ...
      logging.warning("got connection error when trying to connect to consul, retrying")
    except consul.ConsulException:
      logging.warning("got consul error when trying to get leader, retrying")
    time.sleep(0.1)
...
def waitForSession(args):
  ...
  while True:
    ...
    try:
      ...
      logging.warning("got consul error when trying to create session, retrying")
    finally:
  ...
  while True:
    ...
    try:
      ...
      while not acquired:
        ...
        wait = session_ttl * 0.8 if session_ttl is not None else None
        index, data = c.kv.get(lockKey, index=index, wait=wait)
      ...
      logging.warning("got consul error in locked_action, retrying")
...
def waitUntilValue(args):
  ...
  while True:
    try:
      ...
      logging.warning("got consul error in waitUntilValue, retrying")
    except ConnectionError:
      logging.warning("got connection error when trying to connect to consul, retrying")
    time.sleep(0.1)
...
def waitUntilService(args):
  ...
  while True:
    try:
      ...
      logging.warning("got consul error while waiting for service, retrying")
    time.sleep(0.1)
